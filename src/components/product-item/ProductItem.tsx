...
const ProductItem: React.FC = () => {
    const [currentItem, setCurrentItem] = useState<IProductItemProps | undefined>(undefined);
    const titleRef = useRef<HTMLDivElement>(null);
    const [isOverflowTitle, setIsOverflowTitle] = useState<boolean>(false);

    useEffect(() => {
        const el = titleRef.current;
        if (el) {
            const isOver = el.scrollHeight > el.clientHeight + 1;
            setIsOverflowTitle(isOver);
        }
    }, []);
    ...
    const productTitle = useMemo(() => {
        return (
            <div
                ref={titleRef}
                className="line-clamp-2 !mb-2 !px-[10px] overflow-hidden cursor-pointer hover:!text-[#77e322]"
                style={{
                    display: "-webkit-box",
                    WebkitLineClamp: 2,
            ...
            >
                SẢN PHẨM KALENJI Giày chạy bộ JOGFLOW 500.1 cho nam 2
            </div>
        );
    }, []);
    return (
        <div className="!bg-white h-[440px] w-50 overflow-hidden group">
            <div className="w-full h-full flex flex-col justify-start">
                ...
                {isOverflowTitle ? (
                    <Tooltip title={"SẢN PHẨM KALENJI Giày chạy bộ JOGFLOW 500.1 cho nam 2"} color={"#002d3a"}>
                        {productTitle}
                    </Tooltip>
                ) : (
                    productTitle
                ...
                <div className="w-full !px-2.5">
                    ...
                    <div className="w-full !mb-3 flex items-center justify-center gap-1 !text-[#333] !bg-[#a2ff00] !px-2 !py-2 transition-colors duration-300 hover:!bg-[#002932] hover:!text-white cursor-pointer">
                    ...
                    <div className="w-full !mt-4">
                        ...
                        <div className="!mt-1 text-[14px]">Đã bán <span className="font-semibold">20 sản phẩm</span></div>
                    </div>
                </div>
            </div>
        </div>
