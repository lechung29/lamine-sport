...
const SportForWomenList: React.FunctionComponent<IListProps> = (props) => {
    ...
    const navigateToProduct = () => navigate(`/products?gender=${ProductGender.Female}`);
    ...
    React.useEffect(() => {
        const getTopSaleProductByGender = async () => {
            ...
            const data = await ProductService.getTopSaleProductByGender(ProductGender.Female);
    ...
    return (
        <Container padding={[30, 0, 30, 0]} className="!bg-no-repeat !bg-center !bg-cover w-full">
            <Flex justify="space-between" className="w-full !px-4 sm:!px-8 lg:!px-[45px] flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-3">
                <Flex vertical wrap justify="center" className="w-full lg:w-auto lg:max-w-[280px] lg:shrink-0">
                    <Text
                        ...
                        padding={[0, 0, 8, 0]}
                        fontWeight="semibold"
                        className="!text-[#333] hover:!text-[#a2ff00] oswald-font cursor-pointer"
                        titleText={"Thể thao cho nữ"}
                    />
